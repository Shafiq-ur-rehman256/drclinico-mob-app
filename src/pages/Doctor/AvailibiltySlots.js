...
import { getDotorAvailiblitySlots, patientBookAppointment } from "../../services/api.doctor..service";
...
export default function AvailiblitySlots({ navigation, route }) {
  ...
  const bookAppointment = async(data) => {
    ...
    let time = `${data.day_name} (${data.start} - ${data.end})`
    const user = JSON.parse(await AsyncStorage.getItem('user'))
    
    const payload = {
      patient_id: user.id,
      doctor_id: data.doctor.id,
      selected_slot_time: `${data.start}-${data.end}`,
      scheduled_date: data.date
    }

    console.log(payload);
    const response = await patientBookAppointment(payload);
    if(response.code == 0) time = response.msg
      ...
      <View style={styles.doctorList}>
        <ScrollView>
          {slots.map((doctor, index) => {
            return (
              <Pressable key={index} onPress={() => bookAppointment(doctor)}>
